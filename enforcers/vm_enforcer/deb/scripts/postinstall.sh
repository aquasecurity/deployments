...
edit_templates_deb() {
  ...
  sed "s|HOSTNAME=.*\"|HOSTNAME=$(hostname)\"|;
    ...
    s|AQUA_PRIVATE_KEY=.*\"|AQUA_PRIVATE_KEY=${AQUA_PRIVATE_KEY}\",\"AQUA_ENFORCER_TYPE=host\"|;
    s|\"limit\"\:.*|\"limit\"\: ${AQUA_MEMORY_LIMIT}|;
    s|\"period\"\:.*|\"period\"\: 100000|;
    s|\"quota\"\:.*|\"quota\"\: ${AQUA_QUOTA_CPU_LIMIT},|" ${TEMPLATE_DIR}/${ENFORCER_RUNC_CONFIG_TEMPLATE} >${RUNC_TMP_DIRECTORY}/${ENFORCER_RUNC_CONFIG_FILE_NAME}
